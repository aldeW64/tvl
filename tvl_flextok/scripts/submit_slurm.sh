...
#   tvl_flextok/scripts/submit_slurm.sh generation ALIGNMENT_CKPT=/path/to/checkpoint_best_joint.pth
#   tvl_flextok/scripts/submit_slurm.sh generation_vis GENERATION_CKPT=/path/to/checkpoint_best_generation.pth
...
case "$JOB" in
    ...
    alignment_4gpu|multigpu)
        ...
        ;;
    generation|gen)
        SCRIPT="$SCRIPT_DIR/train_generation.sh"
        ;;
    generation_vis|visualize_generation)
        SCRIPT="$SCRIPT_DIR/visualize_generation.sh"
        ;;
    prefix_vis|visualize_prefix)
        SCRIPT="$SCRIPT_DIR/visualize_prefix_recon.sh"
        ;;
    *)
        echo "Usage: $0 {alignment|alignment_4gpu|generation|generation_vis|prefix_vis} [KEY=VALUE ...]"
        exit 2
        ;;
esac
